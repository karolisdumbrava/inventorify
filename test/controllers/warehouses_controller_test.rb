...
class WarehousesControllerTest < ActionDispatch::IntegrationTest
  setup do
      ...
      sign_in @user
      @warehouse = FactoryBot.create(:warehouse, manager: @user)
  end
  ...
  test "should create warehouse" do
      assert_difference('Warehouse.count') do
          post warehouses_url, params: { warehouse: { name: @warehouse.name, address: @warehouse.address, capacity: @warehouse.capacity } }
      end
  ...
  test "should update warehouse" do
      patch warehouse_url(@warehouse), params: { warehouse: { name: @warehouse.name, address: @warehouse.address, capacity: @warehouse.capacity } }
      assert_redirected_to warehouses_url
  end
